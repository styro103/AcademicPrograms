...
class CE
{
	...
	static char encode(char x) //Encode Function for Affine, Vigenère, and Shift Ciphers
	{
	  int y;
	  if(('a'<=x && x<='z')||('A'<=x && x<='Z'))
	  {
	...
	static char decode(char x) //Decode Function for Affine, Vigenère, and Shift Ciphers
	{
	  int y;
	  if(('a'<=x && x<='z')||('A'<=x && x<='Z'))
	  {
